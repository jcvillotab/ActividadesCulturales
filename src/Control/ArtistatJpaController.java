...
public class ArtistatJpaController implements Serializable {
    ...
    public ArrayList findArtistatsNames(List<Integer> ids) {
        EntityManager em = getEntityManager();
        ArrayList<String> names = new ArrayList();
        try {
            for (int i = 0; i < ids.size(); i++) {
                if(ids.get(i)!=0){
                    names.add((em.find(Artistat.class, ids.get(i))).getNombreArtista());
                }
            }
            return names;
        }catch(Exception e){
            System.out.println("Error buscando nombres: "+e);
            return null;
        } finally {
            em.close();
        }
